/***************************************************************************
 * The contents of this file were generated with Amplify Studio.           *
 * Please refrain from making any modifications to this file.              *
 * Any changes to this file will be overwritten when running amplify pull. *
 **************************************************************************/

/* eslint-disable */
import React from 'react';
import { getOverrideProps } from '@aws-amplify/ui-react/internal';
import { Flex, IconAddBox, Text } from '@aws-amplify/ui-react';
export default function Features2x3(props) {
  const { overrides: overridesProp, ...rest } = props;
  const overrides = { ...overridesProp };
  return (
    <Flex
      gap="10px"
      direction="column"
      width="1440px"
      justifyContent="center"
      alignItems="center"
      position="relative"
      padding="40px 140px 40px 140px"
      backgroundColor="rgba(92.00000211596489,102.00000151991844,112.000000923872,1)"
      {...rest}
      {...getOverrideProps(overrides, 'Flex')}
    >
      <Flex
        gap="24px"
        direction="row"
        justifyContent="center"
        alignItems="center"
        shrink="0"
        alignSelf="stretch"
        position="relative"
        padding="0px 0px 0px 0px"
        {...getOverrideProps(overrides, 'Flex.Flex[0]')}
      >
        <Flex
          gap="24px"
          direction="column"
          width="568px"
          grow="1"
          basis="568px"
          height="552px"
          position="relative"
          padding="0px 0px 0px 0px"
          {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0]')}
        >
          <Flex
            gap="0"
            direction="column"
            shrink="0"
            alignSelf="stretch"
            position="relative"
            padding="24px 24px 24px 24px"
            backgroundColor="rgba(0,0,0,1)"
            {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0].Flex[0]')}
          >
            <Flex
              gap="24px"
              direction="row"
              alignItems="flex-start"
              shrink="0"
              alignSelf="stretch"
              position="relative"
              padding="0px 0px 0px 0px"
              {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0].Flex[0].Flex[0]')}
            >
              <IconAddBox
                width="40px"
                height="40px"
                shrink="0"
                overflow="hidden"
                position="relative"
                padding="0px 0px 0px 0px"
                backgroundColor="rgba(0,0,0,1)"
                color="rgba(255,255,255,1)"
                type="add_box"
                fontSize="40px"
                {...getOverrideProps(
                  overrides,
                  'Flex.Flex[0].Flex[0].Flex[0].Flex[0].IconAddBox[0]'
                )}
              ></IconAddBox>
              <Flex
                gap="8px"
                direction="column"
                width="456px"
                justifyContent="center"
                grow="1"
                basis="456px"
                height="120px"
                position="relative"
                padding="0px 0px 0px 0px"
                {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0].Flex[0].Flex[0].Flex[0]')}
              >
                <Text
                  fontFamily="Inter"
                  fontSize="32px"
                  fontWeight="700"
                  color="rgba(255,255,255,1)"
                  lineHeight="40px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Feature A"
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[0].Flex[0].Flex[0].Flex[0].Text[0]'
                  )}
                ></Text>
                <Text
                  fontFamily="Inter"
                  fontSize="16px"
                  fontWeight="400"
                  color="rgba(255,255,255,1)"
                  lineHeight="24px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  letterSpacing="0.010000000000000009px"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam."
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[0].Flex[0].Flex[0].Flex[0].Text[1]'
                  )}
                ></Text>
              </Flex>
            </Flex>
          </Flex>
          <Flex
            gap="0"
            direction="column"
            shrink="0"
            alignSelf="stretch"
            position="relative"
            padding="24px 24px 24px 24px"
            backgroundColor="rgba(0,0,0,1)"
            {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0].Flex[1]')}
          >
            <Flex
              gap="24px"
              direction="row"
              alignItems="flex-start"
              shrink="0"
              alignSelf="stretch"
              position="relative"
              padding="0px 0px 0px 0px"
              {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0].Flex[1].Flex[0]')}
            >
              <IconAddBox
                width="40px"
                height="40px"
                shrink="0"
                overflow="hidden"
                position="relative"
                padding="0px 0px 0px 0px"
                backgroundColor="rgba(0,0,0,1)"
                color="rgba(255,255,255,1)"
                type="add_box"
                fontSize="40px"
                {...getOverrideProps(
                  overrides,
                  'Flex.Flex[0].Flex[0].Flex[1].Flex[0].IconAddBox[0]'
                )}
              ></IconAddBox>
              <Flex
                gap="8px"
                direction="column"
                width="456px"
                justifyContent="center"
                grow="1"
                basis="456px"
                height="120px"
                position="relative"
                padding="0px 0px 0px 0px"
                {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0].Flex[1].Flex[0].Flex[0]')}
              >
                <Text
                  fontFamily="Inter"
                  fontSize="32px"
                  fontWeight="700"
                  color="rgba(255,255,255,1)"
                  lineHeight="40px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Feature C"
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[0].Flex[1].Flex[0].Flex[0].Text[0]'
                  )}
                ></Text>
                <Text
                  fontFamily="Inter"
                  fontSize="16px"
                  fontWeight="400"
                  color="rgba(255,255,255,1)"
                  lineHeight="24px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  letterSpacing="0.010000000000000009px"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam."
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[0].Flex[1].Flex[0].Flex[0].Text[1]'
                  )}
                ></Text>
              </Flex>
            </Flex>
          </Flex>
          <Flex
            gap="0"
            direction="column"
            shrink="0"
            alignSelf="stretch"
            position="relative"
            padding="24px 24px 24px 24px"
            backgroundColor="rgba(0,0,0,1)"
            {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0].Flex[2]')}
          >
            <Flex
              gap="24px"
              direction="row"
              alignItems="flex-start"
              shrink="0"
              alignSelf="stretch"
              position="relative"
              padding="0px 0px 0px 0px"
              {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0].Flex[2].Flex[0]')}
            >
              <IconAddBox
                width="40px"
                height="40px"
                shrink="0"
                overflow="hidden"
                position="relative"
                padding="0px 0px 0px 0px"
                backgroundColor="rgba(0,0,0,1)"
                color="rgba(255,255,255,1)"
                type="add_box"
                fontSize="40px"
                {...getOverrideProps(
                  overrides,
                  'Flex.Flex[0].Flex[0].Flex[2].Flex[0].IconAddBox[0]'
                )}
              ></IconAddBox>
              <Flex
                gap="8px"
                direction="column"
                width="456px"
                justifyContent="center"
                grow="1"
                basis="456px"
                height="120px"
                position="relative"
                padding="0px 0px 0px 0px"
                {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[0].Flex[2].Flex[0].Flex[0]')}
              >
                <Text
                  fontFamily="Inter"
                  fontSize="32px"
                  fontWeight="700"
                  color="rgba(255,255,255,1)"
                  lineHeight="40px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Feature E"
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[0].Flex[2].Flex[0].Flex[0].Text[0]'
                  )}
                ></Text>
                <Text
                  fontFamily="Inter"
                  fontSize="16px"
                  fontWeight="400"
                  color="rgba(255,255,255,1)"
                  lineHeight="24px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  letterSpacing="0.010000000000000009px"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam."
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[0].Flex[2].Flex[0].Flex[0].Text[1]'
                  )}
                ></Text>
              </Flex>
            </Flex>
          </Flex>
        </Flex>
        <Flex
          gap="24px"
          direction="column"
          width="568px"
          grow="1"
          basis="568px"
          height="552px"
          position="relative"
          padding="0px 0px 0px 0px"
          {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1]')}
        >
          <Flex
            gap="0"
            direction="column"
            shrink="0"
            alignSelf="stretch"
            position="relative"
            padding="24px 24px 24px 24px"
            backgroundColor="rgba(0,0,0,1)"
            {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1].Flex[0]')}
          >
            <Flex
              gap="24px"
              direction="row"
              alignItems="flex-start"
              shrink="0"
              alignSelf="stretch"
              position="relative"
              padding="0px 0px 0px 0px"
              {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1].Flex[0].Flex[0]')}
            >
              <IconAddBox
                width="40px"
                height="40px"
                shrink="0"
                overflow="hidden"
                position="relative"
                padding="0px 0px 0px 0px"
                backgroundColor="rgba(0,0,0,1)"
                color="rgba(255,255,255,1)"
                type="add_box"
                fontSize="40px"
                {...getOverrideProps(
                  overrides,
                  'Flex.Flex[0].Flex[1].Flex[0].Flex[0].IconAddBox[0]'
                )}
              ></IconAddBox>
              <Flex
                gap="8px"
                direction="column"
                width="456px"
                justifyContent="center"
                grow="1"
                basis="456px"
                height="120px"
                position="relative"
                padding="0px 0px 0px 0px"
                {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1].Flex[0].Flex[0].Flex[0]')}
              >
                <Text
                  fontFamily="Inter"
                  fontSize="32px"
                  fontWeight="700"
                  color="rgba(255,255,255,1)"
                  lineHeight="40px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Feature B"
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[1].Flex[0].Flex[0].Flex[0].Text[0]'
                  )}
                ></Text>
                <Text
                  fontFamily="Inter"
                  fontSize="16px"
                  fontWeight="400"
                  color="rgba(255,255,255,1)"
                  lineHeight="24px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  letterSpacing="0.010000000000000009px"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam."
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[1].Flex[0].Flex[0].Flex[0].Text[1]'
                  )}
                ></Text>
              </Flex>
            </Flex>
          </Flex>
          <Flex
            gap="0"
            direction="column"
            shrink="0"
            alignSelf="stretch"
            position="relative"
            padding="24px 24px 24px 24px"
            backgroundColor="rgba(0,0,0,1)"
            {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1].Flex[1]')}
          >
            <Flex
              gap="24px"
              direction="row"
              alignItems="flex-start"
              shrink="0"
              alignSelf="stretch"
              position="relative"
              padding="0px 0px 0px 0px"
              {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1].Flex[1].Flex[0]')}
            >
              <IconAddBox
                width="40px"
                height="40px"
                shrink="0"
                overflow="hidden"
                position="relative"
                padding="0px 0px 0px 0px"
                backgroundColor="rgba(0,0,0,1)"
                color="rgba(255,255,255,1)"
                type="add_box"
                fontSize="40px"
                {...getOverrideProps(
                  overrides,
                  'Flex.Flex[0].Flex[1].Flex[1].Flex[0].IconAddBox[0]'
                )}
              ></IconAddBox>
              <Flex
                gap="8px"
                direction="column"
                width="456px"
                justifyContent="center"
                grow="1"
                basis="456px"
                height="120px"
                position="relative"
                padding="0px 0px 0px 0px"
                {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1].Flex[1].Flex[0].Flex[0]')}
              >
                <Text
                  fontFamily="Inter"
                  fontSize="32px"
                  fontWeight="700"
                  color="rgba(255,255,255,1)"
                  lineHeight="40px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Feature D"
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[1].Flex[1].Flex[0].Flex[0].Text[0]'
                  )}
                ></Text>
                <Text
                  fontFamily="Inter"
                  fontSize="16px"
                  fontWeight="400"
                  color="rgba(255,255,255,1)"
                  lineHeight="24px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  letterSpacing="0.010000000000000009px"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam."
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[1].Flex[1].Flex[0].Flex[0].Text[1]'
                  )}
                ></Text>
              </Flex>
            </Flex>
          </Flex>
          <Flex
            gap="0"
            direction="column"
            shrink="0"
            alignSelf="stretch"
            position="relative"
            padding="24px 24px 24px 24px"
            backgroundColor="rgba(0,0,0,1)"
            {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1].Flex[2]')}
          >
            <Flex
              gap="24px"
              direction="row"
              alignItems="flex-start"
              shrink="0"
              alignSelf="stretch"
              position="relative"
              padding="0px 0px 0px 0px"
              {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1].Flex[2].Flex[0]')}
            >
              <IconAddBox
                width="40px"
                height="40px"
                shrink="0"
                overflow="hidden"
                position="relative"
                padding="0px 0px 0px 0px"
                backgroundColor="rgba(0,0,0,1)"
                color="rgba(255,255,255,1)"
                type="add_box"
                fontSize="40px"
                {...getOverrideProps(
                  overrides,
                  'Flex.Flex[0].Flex[1].Flex[2].Flex[0].IconAddBox[0]'
                )}
              ></IconAddBox>
              <Flex
                gap="8px"
                direction="column"
                width="456px"
                justifyContent="center"
                grow="1"
                basis="456px"
                height="120px"
                position="relative"
                padding="0px 0px 0px 0px"
                {...getOverrideProps(overrides, 'Flex.Flex[0].Flex[1].Flex[2].Flex[0].Flex[0]')}
              >
                <Text
                  fontFamily="Inter"
                  fontSize="32px"
                  fontWeight="700"
                  color="rgba(255,255,255,1)"
                  lineHeight="40px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Feature F"
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[1].Flex[2].Flex[0].Flex[0].Text[0]'
                  )}
                ></Text>
                <Text
                  fontFamily="Inter"
                  fontSize="16px"
                  fontWeight="400"
                  color="rgba(255,255,255,1)"
                  lineHeight="24px"
                  textAlign="left"
                  display="flex"
                  direction="column"
                  justifyContent="flex-start"
                  letterSpacing="0.010000000000000009px"
                  width="456px"
                  shrink="0"
                  alignSelf="stretch"
                  position="relative"
                  padding="0px 0px 0px 0px"
                  children="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam."
                  {...getOverrideProps(
                    overrides,
                    'Flex.Flex[0].Flex[1].Flex[2].Flex[0].Flex[0].Text[1]'
                  )}
                ></Text>
              </Flex>
            </Flex>
          </Flex>
        </Flex>
      </Flex>
    </Flex>
  );
}
